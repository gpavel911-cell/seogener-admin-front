...
export function CounterDetailsModal({ isOpen, isLoading, details, onClose }: CounterDetailsModalProps) {
  const [closingDetails, setClosingDetails] = useState<MetricsCounterDto | undefined>(undefined);
      ...
      {isLoadingForRender ? (
        ...
        <DetailsContent>
          ...
          <DetailGrid>
            ...
            <DetailValue>{detailsForRender.counterId ?? "—"}</DetailValue>
            <DetailLabel>Название</DetailLabel>
            <DetailValue>{detailsForRender.counterName ?? "—"}</DetailValue>
            <DetailLabel>Сайт</DetailLabel>
            <DetailValue>{detailsForRender.siteUrl ?? "—"}</DetailValue>
            <DetailLabel>Провайдер</DetailLabel>
            <DetailValue>{detailsForRender.provider}</DetailValue>
            <DetailLabel>Статус</DetailLabel>
            <DetailValue>{detailsForRender.status ?? "—"}</DetailValue>
            <DetailLabel>Наличие</DetailLabel>
            <DetailValue>{detailsForRender.presence ?? "—"}</DetailValue>
